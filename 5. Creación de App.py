    ...
    while True:
        ...
        if q1_ciudad == "1":  # MADRID
            ...
            break
                ...
                CLUSTER= q3_recom.replace(",","").replace("-","").replace(" ","").replace("  ","")
...
def filtrado(clúster_asignado):
    datos_kmeans_filtrado = datos_kmeans.loc[(datos_kmeans.loc[:,"Cluster_1"]==int(clúster_asignado)),:].copy()
    
    while True:
        q3 = input(f"¿Qué tipo de alojamiento te gustaría? 1-HABITACION PRIVADA ({datos_kmeans_filtrado.loc[datos_kmeans_filtrado['Room Type'] == 0].shape[0]} resultados), 2-CASA ENTERA({datos_kmeans_filtrado.loc[datos_kmeans_filtrado['Room Type'] == 1].shape[0]} resultados)")
        ...
        if q3 != "1" and q3 != "2":
            ...
            break
    while True: 
        q4= input(f"Los precios se encuentran entre "+ str(min(datos_kmeans_filtrado["Price"])) +" y "+str(max(datos_kmeans_filtrado["Price"]))+" euros. Elija un precio mínimo: ")
        q5 = input("Elija un precio máximo: ")
        
        if type(clúster_asignado) == str:  
            q6 = input("Introduce el número de huéspedes: ")
            filtro = datos_kmeans_filtrado.loc[((datos_kmeans_filtrado.loc[:,"Price"]>=int(q4))&(datos_kmeans_filtrado.loc[:,"Price"]<=int(q5))&(datos_kmeans_filtrado["Room Type"] == int(q3)-1)&(datos_kmeans_filtrado.loc[:,"Accommodates"]==int(q6))),:].index.tolist()
      
        else:
            filtro = datos_kmeans_filtrado.loc[((datos_kmeans_filtrado.loc[:,"Price"]>=int(q4))&(datos_kmeans_filtrado.loc[:,"Price"]<=int(q5))&(datos_kmeans_filtrado["Room Type"] == int(q3)-1)),:].index.tolist()
            
        if len(filtro)!=0:
                
            break
        else: 
            print("No se han encontrado apartamentos para ese rango de precio, por favor marque una nueva cifra")
    ...
    if type(clúster_asignado) == int:    
        ...
        while True:
            ...
            if pref == "0":
                ...
                RECOMENDACIÓN= datos_kmeans.loc[LISTILLA_INDEX,:].sort_values(by ="dist_cluster_1").copy()
                break
        ...
        for i in range(len(clúster_asignado)):
            ...
            if i ==1:
                ...
                if clúster_asignado[i] == "0":
                    ...
                    RECOMENDACIÓN= datos_kmeans.loc[LISTILLA_INDEX,:].sort_values(by ="dist_cluster_2").copy()
                    break
                ...
                if clúster_asignado[i] == "1":
                    ...
                    RECOMENDACIÓN= datos_kmeans.loc[a,:].sort_values(by ="dist_cluster_2").copy()
                    break
...
dataset.Namecoord = dataset.loc[dataset["ID"] == primer_resultado,"Geolocation"].iloc[0].split(",")
...
folium.Marker(coord, popup=folium.Popup(max_width=20), tooltip=tooltip).add_to(mapi)
